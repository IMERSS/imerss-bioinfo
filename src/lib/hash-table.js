// Appropriated by AMB from https://github.com/ronomon/hash-table/blob/master/index.js
// with minor fixes to adapt Buffer to Uint8Array for browser compatibility and a file scope
...
// Interleaved entropy table used by tabulation hash function:
    var TABLE = (function () {
        //var word = 4;
        var table = new Int32Array(32 * 256 * 2);
        // AMB 20/02/24 Patch for browser
...
// Constants:
    ...
    // Patched AMB from https://stackoverflow.com/questions/17823225/do-arraybuffers-have-a-maximum-length
    HashTable.BUFFER_MAX = 2145386496;
...
// The number of buffers required to support elements at 100% load factor:
    ...
    function Table(keySize, valueSize, bucket, buckets) {
        ...
        // AMB 20/02/24 Patch for browser
        this.buffer = new Uint8Array(this.bucket * this.buckets);
...
// Reset a filter to remove stale entries:
    ...
    Table.prototype.resize = function (resizeBuckets) {
        ...
        // AMB Patch for browser
        this.buffer = new Uint8Array(this.bucket * resizeBuckets);
